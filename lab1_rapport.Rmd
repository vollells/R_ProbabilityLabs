...
sum(Ysim == 0)
```
Antal dragningar där y = 0 är 3539, så andelen blir 0.3539, men det varierar (med ca 100) mellan olika dragningar, såklart.
...
P(X < 0) = / pnorm(0, 0, 1) / = 0.5

P(-1 < X < 1) = / pnorm(1, 0, 1)  - pnorm(-1, 0, 1) / = 0.6826

P(1.96 < X) = / pnorm(1.96, 0, 1) / = 0.9750

P(0 < Y < 10) = / pbinom(9, 10, 0.1) - pbinom(9, 10, 0.1) / = 0.6513

P(Y = 0) = / pbinom(9, 10, 0.1) / = 0.3487

P(0 <= Y <= 10) = / pbinom(9, 10, 0.1) / = 1 
...
```{r }
faults_old <- rbinom(10000, 337, 0.1)
sum(faults_old)/10000
...
```{r }
sum(faults_old < faults_new)/10000 # 0.41
```

### 3)

```{r }
sum(faults_old > 50)/10000
...
X - Bin(n=10,p=0.2)
Y - Gamma(alpha=2,beta =2)

### 1) 

E(X) = n * p = 10 * 0.2 = 2
E(Y) = alpha / beta = 2 / 2 = 1
